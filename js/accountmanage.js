...
// Returns past, present and future events associated with the account. Enables validated user to request a cancellation.
function GetEventsforAccount(){
  //Start by clearing our Events table
  $("#EventTable").empty();
  GetContactsForAccount(); // grab a list of contacts
  var accountID = $("#CompanySearchSelect").val(); // .val grabs the accountID associated with the name
  var accountEventsURL = "https://oit-phl-oit-sandbox.cs17.force.com/rows/services/apexrest/rowseventsbyacc";
  var securityCode = $("#CodeInput").val();
